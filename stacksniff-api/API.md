...
- **On localhost:** Generate or retrieve it using the Django management command:
  ```bash
  uv run python manage.py drf_create_token <username>
  ```
- **In production:** The token is printed in the terminal at the end of `setup.sh` or saved to `/opt/stacksniff/API_TOKEN.txt`
...
3. Set your variables:
   ...
   - `token`: Paste the DRF auth token generated by running `uv run python manage.py drf_create_token <username>` in your terminal.
